  ...
  return (
    <div className='PaymentOrder'>
        ...
        <div className='OrderConfirm'>
            <Button onClick={async() => {((props.adress != ""  && props.validity  != "" && props.CVV  != "" && props.number != ""  && props.name != ""  && props.surname != ""  && props.patronymic != ""  && props.phone != "" ) ?(await setDoc(doc(firestore,"UserBasket",uid,"Order",id), { id:id, order: Basket, date: date, process: "Оплачен", cost: Basket.reduce((i , {cost}) => i+cost,0), adress: props.adress , validity: props.validity, CVV: props.CVV, number: props.number, name: props.name, surname: props.surname, patronymic: props.patronymic,  phone: props.phone }), dispatch(setOrder({orders:await getOrder(uid) })), navigate("/")) : (setErrorinput("Вы ввели не все данные!!"))); const toDelete = await getDocs(docRef);
